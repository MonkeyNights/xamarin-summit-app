...
namespace TheContent.Extensions
{
    /// <summary>
    /// Refer to https://github.com/tpeczek/Demo.Azure.Functions.GraphQL/blob/master/Demo.Azure.Functions.GraphQL/Infrastructure/GraphQLExecuterExtensions.cs 
    /// </summary>
    internal static class GraphQLExecuterExtensions
    {
        ...
        public async static Task<ExecutionResult> ExecuteAsync(this IGraphQLExecuter graphQLExecuter, HttpRequest request)
        {
            ...
            else if (HttpMethods.IsPost(request.Method))
            {
                ...
                {
                    throw new Exception($"Could not parse 'Content-Type' header value '{request.ContentType}'.");
                }
                ...
                switch (mediaTypeHeader.MediaType)
                {
                    ...
                    case FORM_URLENCODED_MEDIA_TYPE:
                        ...
                        break;
                    default:
                        throw new Exception($"Not supported 'Content-Type' header value '{request.ContentType}'.");
                }
            }
